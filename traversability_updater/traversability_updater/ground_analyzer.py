...
class GroundAnalyzer():
    def __init__(self, img_mode='HC'):
        ...
        if img_mode == 'VAE':
            ...
            self.vae.load_state_dict(torch.load("/home/migueldm/local_nav_ws/src/global_navigation/traversability_updater/traversability_updater/VAEmodel_h_16_128.pkl"))
            self.vae.cuda()
            self.vae.eval()
            print('VAE loaded')
    ...
    def insert_sample_elev(self, grid_map):
        ...
        feature = self.get_elev_features(map)
    ...
    def insert_sample_img(self, grid_map):
        ...
        feature = self.get_img_feature(map)
    ...
    def insert_sample_vae(self, grid_map):
        ...
        feature = self.get_vae_feature(map, map_elev)
    ...
    def recompute_transversality_img(self, msg, threshold = 1.0):
        ...
        map = self.map_rgb_layer_to_numpy(msg, layer_name).astype(np.uint32)

        nav_map =  np.zeros((msg.data[layer_index].layout.dim[0].size, msg.data[layer_index].layout.dim[1].size)).astype(np.float32)
        ...
        for i in np.arange(0, map.shape[0] - self.submap_size_, self.recompute_step_):
            for j in np.arange(0, map.shape[1]- self.submap_size_, self.recompute_step_):
                submap = np.copy(map[i:i+self.submap_size_, j:j+self.submap_size_])
                submap = self.get_rgb_image(submap)
                if np.sum(submap == 0) < self.max_unkown_:
                    if (np.sum(submap == 0) > 0):
                        cv.inpaint(submap, (np.any(submap == 0, axis=2)).astype(np.uint8), 3, cv.INPAINT_NS, dst=submap) # TRY NAVIER STOKES INPAINT_NS instead of TELEA
                    ...
                    nav_map[i:i+self.submap_size_, j:j+self.submap_size_] += ((self.is_feature_img_nav(img_features, self.features_, threshold)))*((self.recompute_step_/self.submap_size_)**2)
    ...
    def recompute_transversality_elev(self, msg, threshold = 0.3):
        ...
        # print('Elev map stats2: ', np.mean(elev_map), np.max(elev_map), np.min(elev_map))

        nav_map_elev =  np.zeros((msg.data[layer_index].layout.dim[0].size, msg.data[layer_index].layout.dim[1].size)).astype(np.float32)
        ...
        for i in np.arange(0, elev_map.shape[0] - self.submap_size_, self.recompute_step_*2):
            for j in np.arange(0, elev_map.shape[1] - self.submap_size_, self.recompute_step_*2):
                submap = np.copy(elev_map[i:i+int(self.submap_size_/2), j:j+int(self.submap_size_/2)])
                if np.sum(np.isnan(submap)) < self.max_unkown_/2:
                    ...
                    nav_map_elev[i:i+int(self.submap_size_/2), j:j+int(self.submap_size_/2)] += ((self.is_feature_elev_nav(elev_features, self.elev_features_, threshold)))*((self.recompute_step_*2/self.submap_size_)**2)
    ...
    def recompute_transversality_vae(self, msg, threshold = 1.5):
        ...
        layer_name = 'RGB'
        layer_index = msg.layers.index(layer_name)
        map_rgb = self.map_rgb_layer_to_numpy(msg, layer_name).astype(np.uint32)
        ...
        map = np.concatenate([map_rgb, np.expand_dims(map_elev, axis=2)], axis=2)

        print('Map stats 1: ', np.mean(map[:,:,0]), np.max(map[:,:,0]), np.min(map[:,:,0]))
        print('Map stats 1: ', np.mean(map[:,:,1]), np.max(map[:,:,1]), np.min(map[:,:,1]))
        print('Map stats 1: ', np.mean(map[:,:,2]), np.max(map[:,:,2]), np.min(map[:,:,2]))
        print('Map stats 1: ', np.mean(map[:,:,3]), np.max(map[:,:,3]), np.min(map[:,:,3]))

        nav_map =  np.zeros((msg.data[layer_index].layout.dim[0].size, msg.data[layer_index].layout.dim[1].size)).astype(np.float32)

        img_tensor = img_tensor = torch.zeros(0, 4, self.submap_size_, self.submap_size_)
        ij_array = []
        for i in np.arange(0, map.shape[0] - self.submap_size_, self.recompute_step_):
            for j in np.arange(0, map.shape[1] - self.submap_size_, self.recompute_step_):
                submap = np.copy(map[i:i+self.submap_size_, j:j+self.submap_size_])
        ...
        ij = 0
        for i, j in ij_array:
                nav_map[i:i+self.submap_size_, j:j+self.submap_size_] += ((self.is_feature_img_nav(feats_vae[ij,:], self.features_, threshold)))*((self.recompute_step_/self.submap_size_)**2)
                ij += 1
        return nav_map
    ...
    def get_img_feature(self, submap, normalize=False):
        image = self.get_rgb_image(submap)[:self.submap_size_, :self.submap_size_, :]
    ...
    def get_img_feature_submap(self, image, normalize=False):
        ...
        if (np.sum(image == 0) > 0) and (np.sum(image == 0) < self.max_unkown_):
            cv.inpaint(image, (np.any(image == 0, axis=2)).astype(np.uint8), 3, cv.INPAINT_NS, dst=image)

        if normalize:
            alpha = 255/(np.max(image) - np.min(image))
            beta = -np.min(image)*alpha
            image = cv.convertScaleAbs(image, alpha=alpha, beta=beta).astype(np.float32)
        image = image.astype(np.float32) / 255.0
        return np.concatenate([self.get_sobel_feat(cv.cvtColor(image, cv.COLOR_RGB2GRAY)), self.get_rgb_feat(image), self.get_hsv_feat(image)])
    ...
    def get_vae_feature_submap(self, img, submap_elev, normalize=False):
        ...
        if ((np.sum(img == 0) > 0) and (np.sum(img == 0) < self.max_unkown_)):
            cv.inpaint(img, (np.any(img == 0, axis=2)).astype(np.uint8), 3, cv.INPAINT_NS, dst=img)

        # Temporal, should be inpainted
        submap_elev = np.nan_to_num(submap_elev, nan=0)[:self.submap_size_, :self.submap_size_]
        ...
        print('Map stats 2: ', np.mean(img[:,:,0]), np.max(img[:,:,0]), np.min(img[:,:,0]))
        print('Map stats 2: ', np.mean(img[:,:,1]), np.max(img[:,:,1]), np.min(img[:,:,1]))
        print('Map stats 2: ', np.mean(img[:,:,2]), np.max(img[:,:,2]), np.min(img[:,:,2]))
        print('Map stats 2: ', np.mean(img[:,:,3]), np.max(img[:,:,3]), np.min(img[:,:,3]))
    ...
    def get_elev_features(self, submap):
        ...
        if (np.sum(np.isnan(submap)) > 0) and (np.sum(np.isnan(submap)) < self.max_unkown_):
            ...
            cv.inpaint(submap, (np.isnan(submap)).astype(np.uint8), 3, cv.INPAINT_NS, dst=submap)
    ...
    def add_feature_elev(self, feature):
        ...
        for feat in self.elev_features_:
            if np.linalg.norm(feat - feature) < self.elev_min_dist_:
                ...
                self.elev_features_ = np.append(self.elev_features_, np.expand_dims(feature, axis=0), axis=0)
